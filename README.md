...
CASE: SELECT name, CASE WHEN condition THEN string ELSE string 
END/END AS alias_name FROM table_name;	
...
-- Combining multiple tables is called joining tables.
JOIN: SELECT * FROM table_name JOIN other_table_name 
-- The JOIN command combines the two specified tables.
ON table_name.column_name = other_table_name.column_name;
-- This gives duplicate columns shared between each table the same name, to avoid ambiguity.
-- JOIN can be made more specific by using table_name.column_name for SELECT.
– Commands included, such as WHERE, will have to use table_name.column_name format.
-- If the two tables have different information in the shared column_name, those rows won’t merge.
...
-- Place your SQL commands between the transaction tags. Increases performance for large databases.
